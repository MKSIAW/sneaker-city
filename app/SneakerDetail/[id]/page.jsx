 ...
 //console.log(useCart());
  ...
  const handleAddToCart = () => {
    if (selectedSize && sneaker) {
      ...
      // addToCart(itemToAdd); // Uncomment and implement as needed
  ...
  <div className="p-4 bg-gray-100">
    ...
    <div className="mt-14">
      ...
      <div className="flex overflow-x-auto space-x-4 pb-4">
        {/* Recommended sneakers display */}
        {recommendedSneakers.map((recSneaker) => (
          <div key={recSneaker.id} className="p-4 flex-none w-56">
            <img
              src={recSneaker.image}
              alt={`${recSneaker.brand} ${recSneaker.model}`}
              className="object-cover rounded-lg h-48 w-full mb-2"
            />
            ...
            <Link href={`/sneaker/${recSneaker.id}`}>
              <button className="mt-2 p-2 bg-green-500 text-white rounded w-full">View Details</button>
            </Link>
          </div>
        ))}
      </div>
